...
function metalMat(color = 0x888888) {
  return new THREE.MeshStandardMaterial({ color, roughness: 0.25, metalness: 0.7 });
}
...
function makeBody(obj: THREE.Object3D, radius: number): Body {
  return {
    mesh:   obj,
    radius,
    vel:    new THREE.Vector3((Math.random() - 0.5) * 0.01, (Math.random() - 0.5) * 0.008, 0),
    rotVel: new THREE.Vector3((Math.random() - 0.5) * 0.006, (Math.random() - 0.5) * 0.009, (Math.random() - 0.5) * 0.004),
...
export function useScene(canvasRef: Ref<HTMLCanvasElement | null>) {
  ...
  async function setup() {
    ...
    stats.dom.style.cssText = "position:fixed;top:68px;left:8px;z-index:9999;display:none;opacity:0.85;pointer-events:none;";
    ...
    knotMesh.scale.setScalar(1.1);
    knotMesh.position.set(0.3, 0.0, 0);
    scene.add(knotMesh);
    bodies.push(makeBody(knotMesh, 1.25));
    ...
    function loadGLTF(
      ...
      const ph = new THREE.Mesh(
        placeholderGeo,
        new THREE.MeshStandardMaterial({ color: 0x888888, wireframe: true, transparent: true, opacity: 0.3 })
      );
        ...
        (gltf) => {
          ...
          root.traverse((child) => {
            if ((child as THREE.Mesh).isMesh) {
              (child as THREE.Mesh).material = metalMat(0x888888);
            }
          });
    ...
    /* ── 2. Suzanne ────────────────────────────────────────────────── */
    loadGLTF("/models/suzanne.glb", 1.4, -2.0,  0.8, 0.8,
      new THREE.IcosahedronGeometry(0.7, 1));

    /* ── 3. Labubu ─────────────────────────────────────────────────── */
    loadGLTF("/models/labubu.glb",  1.2,  2.0, -0.6, 0.75,
      new THREE.SphereGeometry(0.6, 8, 6));

    /* ── 4. Gear ───────────────────────────────────────────────────── */
    loadGLTF("/models/gear.glb",    1.2, -1.0, -1.2, 0.7,
      new THREE.TorusGeometry(0.55, 0.18, 6, 14));

    /* ── 5. Toupie ─────────────────────────────────────────────────── */
    loadGLTF("/models/toupie.glb",  1.0,  1.4,  1.0, 0.65,
    ...
    let scrollY = 0, prevScroll = 0;
    const onLenisScroll = (lenis: { scroll: number }) => { scrollY = lenis.scroll; };
    ...
    renderer.setAnimationLoop(() => {
      ...
      for (let i = 0; i < bodies.length; i++) {
        ...
        const pad = b.radius;
        if (b.mesh.position.x >  bx - pad) { b.mesh.position.x =  bx - pad; b.vel.x *= -BOUNCE; }
        if (b.mesh.position.x < -bx + pad) { b.mesh.position.x = -bx + pad; b.vel.x *= -BOUNCE; }
        if (b.mesh.position.y >  by - pad) { b.mesh.position.y =  by - pad; b.vel.y *= -BOUNCE; }
        if (b.mesh.position.y < -by + pad) { b.mesh.position.y = -by + pad; b.vel.y *= -BOUNCE; }
      }
